...
主窗口与 Agents 会话的消息列表使用 **`flex-direction: column-reverse`**、发送时 **`SEND_MESSAGE` 触发 `scrollTo({ top: 0 })`**，以及数据层 **「新在前」** 与布局对齐。流式输出阶段**没有**逐块全局滚动；与用户上滑后是否仍跟流相关的边界见专题文档。

详见：[10-主窗口聊天消息列表与滚动](./10-主窗口聊天消息列表与滚动/README.md)。
