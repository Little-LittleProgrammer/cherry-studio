...
- `stopWhen` 使用 `stepCountIs(N)` 限制最大工具调用次数，N 来自助手的 `maxToolCalls` 配置（可配置）。
- timeout 使用 `IdleTimeoutController`（可重置的超时控制器），每次收到新 chunk 时重置计数器，避免模型在工具调用中间步骤时被误判超时。
...
1. `AiProvider.completions()` 被 `ApiService` 调用。
2. 检查 developer mode → 决定走 `_completionsForTrace()` 或 `modernCompletions()`。
3. `buildPlugins` 按配置装配插件。
4. `createExecutor` 创建 aiCore 执行器。
5. 统一走 `executor.streamText(...)`（文本链路）。
6. 若有 `onChunk`：创建 `AiSdkToChunkAdapter` 并 `processStream()`。
7. 若无 `onChunk`：通过 `consumeStream()` 消费流并保留真实错误诊断。
...
- `AiProvider.generateImage()` 使用 AI SDK `executor.generateImage()` 实现。
- `AiProvider.editImage()` 通过传入 `prompt.images` 参数实现图像编辑。
- 不再回退 legacy 图像链路，全部统一走 AI SDK v6 模式。
