...
`messageThunk -> ApiService -> ModernAiProvider -> RuntimeExecutor -> Provider API -> AiSdkToChunkAdapter -> StreamProcessingService -> UI`
...
1. 为什么 `ModernAiProvider` 是“策略汇聚点”？
...
3. `ModernAiProvider` 适配 provider 配置并组装插件
...
3. 看 `ModernAiProvider` 是否走错 provider 或错误分支（如图像回退）
