...
- `core/providers/`：基于 Extension 的 Provider 注册和实例化（`ExtensionRegistry`、`ProviderExtension`、9 核心 Provider Extension），含 toolFactory（webSearch/urlContext 等能力）
...
当前代码已经稳定在“ExtensionRegistry + ProviderExtension”的 Provider 架构上。阅读时优先以 `packages/aiCore/src/core/providers/` 下的当前实现为准，而不要依赖早期 RegistryManagement 时期的历史理解。
...
- `KnowledgeService.ts`：RAG 知识库构建、检索、预处理与可选重排
- `memory/MemoryService.ts`：长期记忆的提取、存储、去重和检索
- `agents/services/*`：Agent/Session/Message 持久化、Claude Code 会话执行、渠道接入、定时任务与 CherryClaw
